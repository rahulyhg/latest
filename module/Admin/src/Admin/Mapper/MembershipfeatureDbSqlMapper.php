...
class MembershipfeatureDbSqlMapper implements MembershipfeatureMapperInterface {
...
//        exit;
        //if(isset($status)){
        $statement = $this->dbAdapter->query("SELECT tbl_membership_features.*, tbl_membership_package.package_name AS package_name,
            tal.id as adminid,
            tal.username as username
            FROM tbl_membership_features 
                INNER JOIN tbl_membership_package ON tbl_membership_features.package_id = tbl_membership_package.package_id
                LEFT JOIN tbl_admin_login as tal ON tal.id=tbl_membership_features.modified_by OR tal.id=tbl_membership_features.created_by ORDER BY order_val ASC;
                ");
...
//        );
        ...
        //}
        if ($result instanceof ResultInterface && $result->isQueryResult() && $result->getAffectedRows()) {
            $resultSet = new HydratingResultSet($this->hydrator, new Membershipfeatures());
